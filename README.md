...
-   To continuously build the project, run `yarn watch` (_but this will not build new files and folders that you create in case of development_).
...
-   Hit Enter to stat the dev mode
-   Run `loadDir <path>` in the workerB action bar. `path` is the path of the build/dist folder generated by yarn.
...
-   To remove the dist folder and script use `removeDir` command
